...
Stm* Parser::parseStatement() {
    ...
    if (match(Token::ID)) {
        ...
        if (!match(Token::IN)) {
            cout << "Error: se esperaba 'in' después del identificador en 'for'" << endl;
            exit(1);
        }
        if (!match(Token::RANGE)) {
            cout << "Error: se esperaba 'range' después de 'in' en 'for'" << endl;
            exit(1);
        }
        if (!match(Token::PI)) {
        ...
        Exp* start = parseAExp();
        if (!match(Token::COMA)) {
            cout << "Error: se esperaba ',' después del primer argumento en 'range'" << endl;
            exit(1);
        }
        Exp* end = parseAExp();
        if (!match(Token::COMA)) {
            cout << "Error: se esperaba ',' después del segundo argumento en 'range'" << endl;
            exit(1);
        }
        Exp* step = parseAExp();

        if (!match(Token::PD)) {
            cout << "Error: se esperaba ')' después del tercer argumento en 'range'" << endl;
            exit(1);
        }
        ...
        if (!match(Token::ENDFOR)) {
            cout << "Error: se esperaba 'endfor' al final del ciclo for" << endl;
            exit(1);
        }

        s=new ForStatement(lex,start, end, step, body);
    }
    else {
...
// parseBExp: reconoce 'not' o CExp
Exp* Parser::parseBExp() {
    if (match(Token::NOT)) {
        Exp* e = parseCExp();
        BinaryOp op=NOT_OP;
        Exp* left=new BinaryExp( nullptr,e,op);
        return left;
    } else {
        return parseCExp();
    }
